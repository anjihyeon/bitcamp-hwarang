...
import hwarang.artg.funding.service.FundingOrderServiceImpl;
...
public class MemberController {
	...
	@Autowired 
	private FundingOrderServiceImpl orderservice;
...
//		SecurityContextHolder.getContext().setAuthentication(authentication);
	...
	public String showMyPage(Principal principal,Model model) {
		...
		model.addAttribute("orders", orderservice.selectAll(id));
		return "/member/myPage";
	}
	@PreAuthorize("isAuthenticated()")
	@RequestMapping("/pwForm")
	public String showPwForm(Model model, Principal principal) {
		String id = principal.getName();
		MemberVO member = service.memberGetOne(id);
	...
	@PreAuthorize("isAuthenticated()")
	@RequestMapping("/pw")
	public String showPw(Model model,String id,String password) {
		MemberVO member = service.memberGetOne(id);
		member.setMember_id(id);
		member.setMember_password(pwencoder.encode(password));
		service.memberModifyPw(member);
	...
	public String showModify(String id,String SNSid,String password,String name,String tel1,String tel2,String tel3,
		...
		if(id.equals("간편 로그인")) {
			...
			MemberVO member = service.memberGetOne(id);
			if(pwencoder.matches(password,member.getMember_password())) {
				member.setMember_id(id);	
				member.setMember_password(pwencoder.encode(password));
